  ...
  for (let i = 0; i < args.length; ++i) {
    ...
    const handleKnob = (ac: ArgConf) => {
      if (ac.argType && ac.argType !== ArgType.knob) {
        throw new Error(
          "The argument ${arg} was detected as a knob, but is configured as a ${ac.argType}.",
        );
      }
      // lookahead for param
      ...
      if (!oneAheadValue && ac.paramOption === ParamOption.one) {
        throw new Error(
          `The argument ${ac.longName} requires a parameter, but none was supplied.`,
        );
      }
